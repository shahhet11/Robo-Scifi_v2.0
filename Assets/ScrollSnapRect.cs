...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    void Start()
    {
        ...
        if (isWeapon)
        {
            //PlayerPrefs.SetInt("WeaponFirstTime", 0);
            if (PlayerPrefs.GetInt("WeaponFirstTime") == 0)
            {
                WeaponLoadout.LockAll(_pageCount);

                PlayerPrefs.SetInt("Weapon0", 2);
                PlayerPrefs.SetInt("WeaponFirstTime", 1);
            }

            WeaponLoadout.CheckWeapon(startingPage);
            WeaponLoadout.currentEqId = PlayerPrefs.GetInt("selectedWeapon");

            
        }
        else
        {
            //PlayerPrefs.SetInt("TankFirstTime", 0);
            if (PlayerPrefs.GetInt("TankFirstTime") == 0)
            {
                TankManager.LockAll(_pageCount);
                PlayerPrefs.SetInt("Tank0", 2);

                PlayerPrefs.SetInt("TankFirstTime", 1);
            }

            TankManager.CheckTank(startingPage);
        }
    }
    ...
    private void LerpToPage(int aPageIndex)
    {
        ...
        if(isWeapon)
            WeaponLoadout.CheckWeapon(aPageIndex);
        else
            TankManager.CheckTank(aPageIndex);
    }
